...
def ex3() -> None:
	"""
	Faire un programme qui simule le lancer de 3 dés afin d’obtenir en 3 lancers max : 4 2 1
	- À chaque coup on peut garder 1, 2 ou 3 dés et dès que l’on a 3 dés avec 1,2,4 le jeu est terminé.
	Exemple :
	...
	for i in range(games):
		...
		if len(kept_dices) == 3:
			wins += 1
			print_result(f"Partie {i + 1} gagnée en {moves + 1} {plural(moves + 1, 'coup')}")
		else:
			print_result(f"Partie {i + 1} perdue")

	loses: int = games - wins
	percentage: float = round(wins / games * 100, 2)
	print_result(f"Vous avez joué {games} {plural(games, 'partie')}, {wins} {plural(wins, 'gagnée')} pour {loses} {plural(loses, 'perdue')} soit {percentage}% de gain")
